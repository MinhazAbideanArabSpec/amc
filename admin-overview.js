...
function renderContractTimeline(contracts, today) {
  ...
  el.innerHTML = `<div class="timeline-card-inner">` + contracts.map(c => {
    ...
    const daysLabel = daysLeft <= 0 ? t('contract_expired') : `${daysLeft} ${t('contract_days_left')}`;
...
async function loadCustomerContracts(customerId) {
  ...
  let { data: contracts, error } = await sb
    .from('contracts')
    .select('*')
  ...
  overviewContainer.innerHTML = contracts.map(c => {
    ...
    const daysLabel = daysLeft < 0 ? t('contract_expired') : `${daysLeft} ${t('contract_days_remaining')}`;
    const urgentClass = (daysLeft >= 0 && daysLeft <= 60) ? 'urgent' : '';
    return `
      <div class="overview-row">
        <div class="overview-row-main">
          <span class="contract-num">${c.contract_number}</span>
          <span class="badge status-${c.status}">${c.status === 'active' ? t('contract_active') : t('contract_expired')}</span>
          <span class="days-left ${urgentClass}">${daysLabel}</span>
        </div>
        <a class="overview-link" href="javascript:void(0)" onclick="switchCustomerTab('contract')">${t('btn_view_contract')}</a>
      </div>
    `;
  }).join('');
  ...
  container.innerHTML = contracts.map(c => `
    <div class="contract-block" id="contract-${c.id}">
      ...
      <div class="contract-grid">
        ...
        <div class="contract-field"><div class="lbl">${t('contract_start_date')}</div><div class="val">${fmtDate(c.start_date)}</div></div>
        <div class="contract-field"><div class="lbl">${t('contract_end_date')}</div><div class="val">${fmtDate(c.end_date)}</div></div>
...
// KSA-aware "today" midnight for day calculations
function ksaToday() {
  const now = new Date();
...
async function loadCustomerAssets(customerId) {
  ...
  if (!assets && !error) {
    await new Promise(r => setTimeout(r, 1500));
    [{ data: assets, error }, { data: assignments }] = await Promise.all([
      sb.from('assets').select('id, name, employee_name, category, asset_group').eq('customer_id', customerId).order('employee_name'),
      sb.from('asset_status_assignments').select('asset_id, asset_statuses(name, color)').eq('customer_id', customerId).eq('is_resolved', false)
    ]);
  }

  if (error) {
    console.error('loadCustomerAssets error:', error);
    if (listEl) listEl.innerHTML = `<div class="empty-state">Error loading assets.</div>`;
    if (donutEl) donutEl.innerHTML = `<div class="empty-state">Error: ${error.message}</div>`;
    return;
  }
...
async function openAssetDetail(assetId) {
  ...
  const { data: vras } = await sb.from('visit_report_assets')
    .select('*, visit_reports(visit_number, visit_date, engineer_name, report_type)')
    .eq('asset_id', assetId)
    .order('created_at', { ascending: false })
    .limit(1);
  ...
  if (vra) {
    ...
    const reportType = vra.visit_reports?.report_type || (asset.asset_group === 'data_center' ? 'data_center' : 'end_user');
    const activeChecklist = reportType === 'data_center' ? DC_CHECKLIST : CHECKLIST;
    checklistHtml = `
      <div style="margin-top:20px;">
        <div style="font-size:13px;font-weight:700;color:var(--ink);margin-bottom:12px;">Visit Checklist</div>
        ${activeChecklist.map(s => {
