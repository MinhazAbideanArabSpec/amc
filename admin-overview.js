...
async function loadAdminOverview() {
  ...
  const criticalAssignments = (assignments || []).filter(a => a.asset_statuses?.name === 'Critical');
  ...
  document.getElementById('stat-critical-assets').textContent = criticalAssignments.length;
  ...
  const critEl = document.getElementById('admin-critical-tbody');
  if (!criticalAssignments.length) {
    critEl.innerHTML = `<tr><td colspan="3" class="empty-state" style="color:#27AE60;">✓ No critical assets</td></tr>`;
  } else {
    // Fetch failed sections for critical assets
    const assetIds = criticalAssignments.map(a => a.asset_id);
    const { data: vras } = await sb.from('visit_report_assets')
      .select('asset_id, visit_report_checks(section, result)')
      .in('asset_id', assetIds).order('created_at', { ascending: false });
    const latestVraMap = {};
    (vras || []).forEach(v => { if (!latestVraMap[v.asset_id]) latestVraMap[v.asset_id] = v; });

    critEl.innerHTML = criticalAssignments.map(a => {
      const checks = latestVraMap[a.asset_id]?.visit_report_checks || [];
      const failed = checks.filter(c => c.result === 'fail').map(c => c.section.replace(' Check','').replace(' Verification','').replace(' Optimization','').replace(' Inspection','')).join(', ');
      return `<tr>
        <td style="font-weight:600;">${a.assets?.employee_name || a.assets?.name || '—'}</td>
        <td>${a.profiles?.name || '—'}</td>
        <td style="font-size:12px;color:var(--rust);">${failed || '—'}</td>
      </tr>`;
    }).join('');
  }
  ...
  subEl.innerHTML = expSubs.length
    ...
    : `<tr><td colspan="5" class="empty-state">No subscriptions expiring within 60 days.</td></tr>`;
}
