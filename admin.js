...
async function loadCustomerAssets(customerId) {
  ...
  const { data: vras } = await sb.from('visit_report_assets')
    .select('asset_id, overall_status, visit_report_checks(result), visit_reports(visit_date)')
      ...
      const scoreHtml = visitDate ? `
        <div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--line);display:flex;align-items:center;justify-content:space-between;">
          ...
          <div style="display:flex;gap:6px;">
            <span style="font-size:11px;font-weight:700;color:#27AE60;">${pass} Pass</span>
            <span style="font-size:11px;font-weight:700;color:#D4A017;">${ok} OK</span>
            <span style="font-size:11px;font-weight:700;color:#C0392B;">${fail} Fail</span>
          </div>
        </div>` : `<div style="margin-top:6px;font-size:11px;color:#9CA3AF;">No visit yet</div>`;
    ...
    checklistHtml = `
      <div style="margin-top:20px;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:8px;">
          <div style="font-size:13px;font-weight:700;color:var(--ink);">Visit Checklist</div>
          <div style="font-size:12px;color:#8A8377;">
            ${visit?.visit_number || ''} · ${fmtDate(visit?.visit_date)} · ${visit?.engineer_name || ''}
          </div>
        </div>
        ${CHECKLIST.map(s => {
