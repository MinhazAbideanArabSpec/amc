  ...
  const tiles = FIXED_ORDER.map(label => {
    ...
    return `
      <div style="
        ...
        border-radius: 12px;
        padding: 20px;
        background: ${active ? style.hex : style.bg};
        border: 1.5px solid ${active ? style.hex : style.hex + '40'};
        display: flex;
      ...
      ">
        <div style="font-size:36px;font-weight:800;color:${active ? '#fff' : style.hex};line-height:1;">${count}</div>
        <div style="font-size:12px;font-weight:700;color:${active ? 'rgba(255,255,255,0.9)' : style.hex};margin-top:6px;">${label}</div>
        <div style="font-size:11px;color:${active ? 'rgba(255,255,255,0.6)' : style.hex + '80'};margin-top:3px;">${pct}%</div>
      </div>`;
  }).join('');
