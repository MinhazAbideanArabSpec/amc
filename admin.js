...
async function loadCustomerAssets(customerId) {
  ...
  if (error) {
    if (summaryEl) summaryEl.innerHTML = `<div class="empty-state">Error: ${error.message}</div>`;
    if (dashList2) dashList2.innerHTML = `<div class="empty-state">Error loading assets.</div>`;
    return;
  }
  if (!assets || !assets.length) {
    if (summaryEl) summaryEl.innerHTML = `<div class="empty-state">No assets registered yet.</div>`;
    const dl = document.getElementById('customer-assets-list-dash');
    if (dl) dl.innerHTML = `<div class="empty-state">No assets registered yet.</div>`;
  ...
  // Populate dashboard overview copy (always visible)
  const dashList = document.getElementById('customer-assets-list-dash');
  if (dashList) dashList.innerHTML = assetCardsHtml;
  // Populate tab list if visible (may not exist if tab not yet opened)
