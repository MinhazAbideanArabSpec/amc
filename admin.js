// admin.js — admin overview, users, contracts, assets
    ...
    overviewContainer.innerHTML = contracts.map(c => {
      ...
      return `
        <div class="overview-row">
          ...
          <a class="overview-link" href="#contract-${c.id}">View full contract →</a>
        </div>
      `;
    }).join('');
  ...
  let allHealthStatuses = []; // cached: [{id, category, label, color, sort_order}]
  ...
  async function saveAsset() {
    ...
    const payload = { customer_id, employee_name, name, category, serial_model, location, notes };
  ...
  async function loadCustomerAssets(customerId) {
    ...
    listEl.innerHTML = `<div class="asset-card-grid">` + assets.map(a => {
      const statuses = assetStatusMap[a.id] || [];
      const pills = statuses.length
        ? statuses.map(s => `<span class="status-pill c-${s.color}" style="margin-right:4px;">${s.name}</span>`).join('')
