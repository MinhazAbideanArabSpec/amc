...
function renderAssetDonut(counts, total) {
  ...
  const TILE_STYLES = {
    'Critical':        { hex: '#C0392B', bg: '#FDEDEC', icon: '🔴' },
    'Warning':         { hex: '#D4A017', bg: '#FEF9E7', icon: '🟡' },
    'Pass':            { hex: '#27AE60', bg: '#EAFAF1', icon: '🟢' },
    'No Active Status':{ hex: '#94A3B8', bg: '#F8FAFC', icon: '⚪' },
  };

  const tiles = Object.entries(counts)
    .sort((a, b) => {
      const order = { 'Critical': 0, 'Warning': 1, 'Pass': 2, 'No Active Status': 3 };
      return (order[a[0]] ?? 9) - (order[b[0]] ?? 9);
    })
    .map(([label, { count }]) => {
      const style = TILE_STYLES[label] || { hex: '#94A3B8', bg: '#F8FAFC', icon: '•' };
      return `
        <div style="flex:1;min-width:100px;border-radius:10px;padding:14px 16px;
          background:${style.bg};border:1.5px solid ${style.hex}30;text-align:center;">
          <div style="font-size:28px;font-weight:800;color:${style.hex};line-height:1;">${count}</div>
          <div style="font-size:11.5px;font-weight:600;color:${style.hex};margin-top:4px;white-space:nowrap;">${label}</div>
          <div style="font-size:10px;color:${style.hex}99;margin-top:2px;">${Math.round(count/total*100)}%</div>
        </div>`;
    }).join('');

  donutEl.innerHTML = `
    <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:10px;">${tiles}</div>
...
async function loadCustomerAssets(customerId) {
  ...
  const { data: vras } = await sb.from('visit_report_assets')
    .select('id, asset_id, overall_status, visit_report_checks(result), visit_reports(visit_date)')
      ...
      const scoreHtml = visitDate ? `
        <div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--line);display:flex;align-items:center;justify-content:space-between;">
          ...
          <div style="display:flex;gap:6px;">
            <span style="font-size:11px;font-weight:700;color:#27AE60;">${pass} ${t('result_pass')}</span>
            <span style="font-size:11px;font-weight:700;color:#D4A017;">${ok} ${t('result_ok')}</span>
            <span style="font-size:11px;font-weight:700;color:#C0392B;">${fail} ${t('result_fail')}</span>
          </div>
        </div>` : `<div style="margin-top:6px;font-size:11px;color:#9CA3AF;">${t('no_visit_yet')}</div>`;

      // Issue tags grouped by section
          ...
          Object.entries(bySection).map(([sec, stags]) => `
            <div style="margin-bottom:5px;">
              ...
              <div style="display:flex;flex-wrap:wrap;gap:3px;">
                ${stags.map(t => `<span class="issue-tag-pill ${isCrit ? 'critical' : ''}">${t.label}</span>`).join('')}
              </div>
            </div>`).join('') + `</div>`;
      }
