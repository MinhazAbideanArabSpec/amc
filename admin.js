...
function renderAssetDonut(counts, total) {
  ...
  const tiles = FIXED_ORDER.map(label => {
    ...
    return `
      <div style="flex:1;min-width:90px;border-radius:10px;padding:14px 16px;
        background:${style.bg};border:1.5px solid ${style.hex}30;text-align:center;">
        <div style="font-size:28px;font-weight:800;color:${count > 0 ? style.hex : '#C0C8D0'};line-height:1;">${count}</div>
        <div style="font-size:11.5px;font-weight:600;color:${count > 0 ? style.hex : '#94A3B8'};margin-top:4px;white-space:nowrap;">${label}</div>
        <div style="font-size:10px;color:${count > 0 ? style.hex : '#94A3B8'}99;margin-top:2px;">${pct}%</div>
      </div>`;
  }).join('');
