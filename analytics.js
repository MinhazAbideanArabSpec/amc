// analytics.js — customer-facing website analytics (Cloudflare Web Analytics)
...
async function loadCustomerAnalyticsTab(customerId) {
  ...
  let result;
  try {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/cloudflare-analytics?customerId=${encodeURIComponent(customerId)}`, {
