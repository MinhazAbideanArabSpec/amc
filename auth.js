...
async function resendOtp() {
  ...
  btn.disabled = true;
  btn.textContent = 'Sending…';
  await sb.auth.signInWithOtp({ email: _otpEmail, options: { shouldCreateUser: false } });
  btn.disabled = false;
...
async function afterLogin() {
  ...
  if (profile.role === 'admin') {
    ...
    loadCustomerIssueTags(profile.id);
  }
}
...
function switchCustomerTab(tab) {
  ...
  if (tab === 'contract') { loadCustomerSubscriptions(myProfile?.id); }
  if (tab === 'assets')   loadGroupAssets('end_user', myProfile?.id);
  if (tab === 'dc')       loadGroupAssets('data_center', myProfile?.id);
}
