      ...
      <tbody>
        ${assets.map(a => {
          ...
          const statusHtml = statuses.length
            ? statuses.map(s => {
                const fixed = FIXED_STATUSES ? FIXED_STATUSES.find(f => f.name === s.name) : null;
                const hex = fixed ? fixed.hex : '#94A3B8';
                return `<span style="font-size:11px;font-weight:700;padding:2px 8px;border-radius:4px;color:${hex};background:${hex}22;">${s.name}</span>`;
              }).join('')
