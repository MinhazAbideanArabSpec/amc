...
async function loadCustomerRenewalsTab(customerId) {
  ...
  let { data: subs, error } = await sb.from('subscriptions')
    ...
    .eq('customer_id', customerId).order('end_date', { ascending: true });
  if (!subs && !error) {
  ...
  if (expiring.length) {
    ...
    html += `<div class="sub-grid">` + expiring.map(s => subCardHtml(s, false, false)).join('') + `</div>`;
  }
  if (active.length) {
    html += `<div style="font-size:11px;font-weight:700;color:var(--sage);text-transform:uppercase;letter-spacing:0.06em;margin-bottom:10px;margin-top:16px;">Active (${active.length})</div>`;
    html += `<div class="sub-grid">` + active.map(s => subCardHtml(s, false, false)).join('') + `</div>`;
  }
  if (expired.length) {
    html += `<div style="font-size:11px;font-weight:700;color:var(--rust);text-transform:uppercase;letter-spacing:0.06em;margin-bottom:10px;margin-top:16px;">Expired (${expired.length})</div>`;
...
async function loadCustomerReportsCount(customerId) {
  const { data } = await sb.from('visit_reports')
    .select('visit_number')
    .eq('customer_id', customerId)
    .eq('status', 'completed');
...
async function loadDashIssueTags(customerId) {
  ...
  // fallback for old single panel
  const legacyEl = document.getElementById('dash-issue-tags-panel');

  const target = euEl || legacyEl;
  if (!target && !dcEl) return;
  ...
  if (!assets?.length) {
    ...
    if (legacyEl) legacyEl.innerHTML = '<div class="empty-state">No assets.</div>';
    return;
  }
  ...
  if (!vraIds.length) {
    ...
    if (legacyEl) legacyEl.innerHTML = '<div class="empty-state">No visit data yet.</div>';
    return;
  }
  ...
  (vitags || []).forEach(vit => {
    ...
    if (!asset) return;
    if (!tagGroups[tagDef.id]) tagGroups[tagDef.id] = { label: tagDef.label, section: tagDef.section, assets: [], assetGroups: new Set() };
  ...
  function renderTagPanel(groupFilter) {
    ...
    if (!filtered.length) return '<div class="empty-state">No issue tags recorded yet.</div>';
    const bySection = {};
        ...
        <div style="display:flex;flex-wrap:wrap;gap:6px;">
          ${tags.map(tag => `
            <button onclick="openTagDetailModal('${tag.tagId}')"
              ...
              color:#2C2C2A;font-size:12px;font-weight:600;cursor:pointer;">
              ${tag.label}
  ...
  if (legacyEl && !euEl) legacyEl.innerHTML = renderTagPanel('end_user') + renderTagPanel('data_center');
}
