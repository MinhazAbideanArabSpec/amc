...
function switchDashIssueTagTab(tab) {
  const euBtn = document.getElementById('dit-tab-eu');
  const dcBtn = document.getElementById('dit-tab-dc');
  const euPanel = document.getElementById('dash-issue-tags-panel-eu');
  const dcPanel = document.getElementById('dash-issue-tags-panel-dc');
  if (!euBtn || !dcBtn) return;
  if (tab === 'eu') {
    euBtn.style.background = 'var(--accent)'; euBtn.style.color = '#fff';
    dcBtn.style.background = '#F8FAFC';       dcBtn.style.color = '#475569';
    if (euPanel) euPanel.style.display = 'block';
    if (dcPanel) dcPanel.style.display = 'none';
  } else {
    dcBtn.style.background = 'var(--accent)'; dcBtn.style.color = '#fff';
    euBtn.style.background = '#F8FAFC';       euBtn.style.color = '#475569';
    if (dcPanel) dcPanel.style.display = 'block';
    if (euPanel) euPanel.style.display = 'none';
  }
}
// ═══════════════════════════════════════════════════════
