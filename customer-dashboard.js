...
async function loadCustomerRenewalsTab(customerId) {
  ...
  let html = '';
  const fullWidthGrid = 'display:grid;grid-template-columns:1fr;gap:10px;';
  if (expiring.length) {
    ...
    html += `<div style="${fullWidthGrid}margin-bottom:16px;">` + expiring.map(s => subCardHtml(s, false, false)).join('') + `</div>`;
  }
  if (active.length) {
    ...
    html += `<div style="${fullWidthGrid}margin-bottom:16px;">` + active.map(s => subCardHtml(s, false, false)).join('') + `</div>`;
  }
  if (expired.length) {
    ...
    html += `<div style="${fullWidthGrid}">` + expired.map(s => subCardHtml(s, false, false)).join('') + `</div>`;
  }
  el.innerHTML = html;
}
