...
async function loadVisitDashboard(customerId) {
  ...
  metaEl.textContent = grandTotal ? `${grandTotal} total checks across all assets` : '';

  // Render section scorecard grid
  if (grandTotal === 0) {
    sectionEl.innerHTML = `<div class="empty-state">${t('no_visit_data')}</div>`;
  } else {
            ...
            <div style="border-radius:8px;padding:10px 12px;background:${secBg};border:1px solid ${secColor}25;">
              ...
              ${noData
                ...
                : `<div style="display:flex;gap:8px;font-size:11px;font-weight:600;">
                    <span style="color:#27AE60;">${sec.pass}P</span>
                    <span style="color:#D4A017;">${sec.ok}OK</span>
                    <span style="color:#C0392B;">${sec.fail}F</span>
                  </div>
