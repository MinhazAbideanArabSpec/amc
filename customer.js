...
async function loadDashIssueTags(customerId) {
  const el = document.getElementById('dash-issue-tags-panel');
  if (!el) return;
  ...
  const { data: assets } = await sb.from('assets')
    .select('id, name, employee_name, category')
    .eq('customer_id', customerId);
  if (!assets?.length) { el.innerHTML = '<div class="empty-state">No assets.</div>'; return; }
  ...
  (vras || []).forEach(v => { if (!latestVraMap[v.asset_id]) latestVraMap[v.asset_id] = v; });
  const vraIds = Object.values(latestVraMap).map(v => v.id);
  if (!vraIds.length) { el.innerHTML = '<div class="empty-state">No visit data yet.</div>'; return; }
  ...
  if (!vitags?.length) { el.innerHTML = '<div class="empty-state">No issue tags recorded yet.</div>'; return; }

  // Build: { tagId: { label, section, assets: [] } }
  const assetMap = {};
  assets.forEach(a => { assetMap[a.id] = a; });
  const vraToAsset = {};
  Object.entries(latestVraMap).forEach(([assetId, vra]) => { vraToAsset[vra.id] = assetId; });

  const tagGroups = {}; // tagId → { label, section, assets[] }
  vitags.forEach(vit => {
    ...
    if (!asset) return;
    if (!tagGroups[tagDef.id]) tagGroups[tagDef.id] = { label: tagDef.label, section: tagDef.section, assets: [] };
  ...
  // Group by section
  const bySection = {};
  Object.entries(tagGroups).forEach(([tagId, data]) => {
  ...
  el.innerHTML = Object.entries(bySection).map(([section, tags]) => `
      ...
      <div style="display:flex;flex-wrap:wrap;gap:6px;">
        ${tags.map(tag => `
          <button onclick="openTagDetailModal('${tag.tagId}')"
            ...
            border-radius:20px;border:1.5px solid #D4A017;background:#FEF9E7;
            color:#92660F;font-size:12px;font-weight:600;cursor:pointer;transition:opacity 0.15s;"
            onmouseover="this.style.opacity='0.75'" onmouseout="this.style.opacity='1'">
            ${tag.label}
            <span style="background:#D4A017;color:#fff;border-radius:10px;font-size:10px;font-weight:700;padding:1px 6px;">${tag.assets.length}</span>
          </button>`).join('')}
      </div>
    </div>`).join('');
}
  ...
  container.innerHTML = reports.map(r => {
    ...
    <div class="report-list-item">
      <div class="report-list-main">
        ...
        ${assetNames ? `<span style="font-size:12px;color:var(--accent);font-weight:600;margin-top:2px;">${assetNames}</span>` : ''}
      </div>
