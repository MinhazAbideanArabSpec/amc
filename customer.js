...
async function loadNotifications(customerId) {
  ...
  const { data: criticals } = await sb.from('asset_status_assignments')
    .select('*, assets(name, employee_name, category), asset_statuses(name, color)')
    .eq('customer_id', customerId)
    .eq('status_id', '00000000-0000-0000-0000-000000000001') // Critical UUID
    .eq('is_resolved', false);
  ...
  if (count > 0) {
    // Red background for card
    if (card) {
      card.style.background = 'rgba(192,57,43,0.25)';
  ...
  // Store for modal
  window._criticalAssets = criticals || [];
}
    ...
    body.innerHTML = `
      ...
      ${criticals.map(a => `
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;
          padding:12px 14px;border-radius:6px;background:#FDEDEC;border:1px solid #f5b7b1;margin-bottom:8px;">
          <div>
            ...
            ${a.notes ? `<div style="font-size:12.5px;color:#922B21;margin-top:6px;font-style:italic;">${a.notes}</div>` : ''}
          </div>
