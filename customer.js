...
function openAssetHealthModal(label) {
  ...
  const group = window._assetHealthGroups?.[label];
  const assets = group?.assets || (window._dashAssetResults || [])
    .filter(r => {
      if (label === 'No Active Status') return !r.vra || !window._assetHealthGroups || Object.entries(window._assetHealthGroups).every(([k]) => k === 'No Active Status' || !(window._assetHealthGroups[k]?.assets || []).find(a => a.id === r.asset?.id));
      return (window._assetHealthGroups[label]?.assets || []).find(a => a.id === r.asset?.id);
    })
    .map(r => r.asset);

  // Fallback: use customerAssetsCache filtered by status
  const displayAssets = group?.assets?.length
    ? group.assets
    : (customerAssetsCache || []).filter(a => {
        const statuses = (window._assetHealthGroups || {});
        if (label === 'No Active Status') {
          return !Object.entries(statuses).some(([k, g]) =>
            k !== 'No Active Status' && (g.assets || []).find(x => x.id === a.id)
          );
        }
        return (statuses[label]?.assets || []).find(x => x.id === a.id);
      });
    ...
    if (sectionEl) sectionEl.innerHTML = `
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
        ${SECTION_NAMES.map(s => {
          ...
          return `
            <div style="border-radius:8px;padding:10px 12px;background:${secBg};border:1px solid ${secColor}25;">
