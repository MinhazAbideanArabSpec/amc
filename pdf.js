  ...
  try {
    ...
    if (window._dashContracts?.length) {
      window._dashContracts.forEach((c, i) => {
        ...
        doc.setFontSize(8);
        doc.setTextColor(...PDF_MUTED);
        doc.text(`${fmtDate(c.start_date)}  →  ${fmtDate(c.end_date)}`, 16, y + 10);
    ...
    // ── Page 2: Asset Statuses ─────────────────────────────
    doc.addPage();
    y = pageTop('Asset Statuses');
    ...
    if (window._statusGroups && Object.keys(window._statusGroups).length) {
      Object.values(window._statusGroups).forEach(g => {
        const assetCount = g.assets.length;
        const rgb = STATUS_RGB[g.status.color] || PDF_MUTED;
        const names = g.assets.map(a => a?.employee_name || a?.name || '—');
        const nameStr = names.join('    ');
        const nameLines = doc.splitTextToSize(nameStr, pw - 32);
        const blockH = 12 + nameLines.length * 5 + 8;

        y = pdfCheckBreak(doc, y, blockH + 4);
        ...
        doc.setFontSize(10);
        doc.setTextColor(255, 255, 255);
        doc.text(`${g.status.name}`, 18, y + 7);
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(8.5);
        doc.text(`${assetCount} asset${assetCount !== 1 ? 's' : ''}`, pw - 16, y + 7, { align: 'right' });
        y += 12;

        // Asset names
        doc.setFillColor(rgb[0] + 50 > 255 ? 255 : rgb[0] + 50,
                         rgb[1] + 50 > 255 ? 255 : rgb[1] + 50,
                         rgb[2] + 50 > 255 ? 255 : rgb[2] + 50);
        doc.rect(14, y - 1, pw - 28, nameLines.length * 5 + 4, 'F');
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(8.5);
        doc.setTextColor(...PDF_DARK);
        doc.text(nameLines, 18, y + 3.5);
        y += nameLines.length * 5 + 8;
      });
    } else {
      doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(...PDF_MUTED);
      doc.text('No active statuses assigned.', 14, y);
    }
    ...
    if (grandTotal > 0 && window._dashSectionTotals) {
      ...
      Object.entries(window._dashSectionTotals).forEach(([section, s], i) => {
        y = pdfCheckBreak(doc, y, 12);
        if (i % 2 === 0) { doc.setFillColor(248, 249, 250); doc.rect(14, y - 2, pw - 28, 11, 'F'); }

        const sResult = s.fail > 0 ? 'fail' : s.ok > 0 ? 'ok' : s.total > 0 ? 'pass' : null;
        const sRgb    = sResult === 'fail' ? PDF_RED : sResult === 'ok' ? PDF_AMBER : sResult === 'pass' ? PDF_GREEN : PDF_MUTED;

        doc.setFont('helvetica', 'bold'); doc.setFontSize(8.5); doc.setTextColor(...PDF_DARK);
        doc.text(section, 16, y + 5);

        if (sResult) {
          pdfResultBadge(doc, sResult, pw - 14 - 22, y + 1);
        }
        ...
        if (s.total > 0) {
          ...
          doc.text('No data', pw - 28, y + 5, { align: 'right' });
        }
        y += 12;
      });
    } else {
