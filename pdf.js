...
async function downloadVisitReportPDF(reportId, visitNum, visitDate, engineerName) {
  ...
  (vras || []).forEach(vra => {
    ...
    CHECKLIST.forEach((s, i) => {
      ...
      const pass = match?.result === 'pass' ? 1 : 0;
      const ok   = match?.result === 'ok'   ? 1 : 0;
      const fail = match?.result === 'fail'  ? 1 : 0;
      y = checkPageBreak(doc, y);
      y = pdfTableRow(doc, [s, pass, ok, fail], y, [100, 22, 22, 22], false, i % 2 === 1);
    });
