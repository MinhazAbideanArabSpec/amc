...
async function loadCustomerStatuses(customerId) {
  ...
  if (!assignments || !assignments.length) {
    el.innerHTML = `<div class="empty-state">${t('no_statuses')}</div>`;
    return;
  }
    ...
    Object.values(groups).map(g => {
      ...
      return `
        <div class="status-tile c-${fixed.color}"
          ...
          <div class="st-name">${t('status_' + g.status.name.toLowerCase()) || g.status.name}</div>
          <div class="st-click">${t('click_to_view')}</div>
        </div>`;
    }).join('') + `</div>`;
