...
async function loadSubscriptions() {
  ...
  if (summaryEl) {
    summaryEl.innerHTML = [
      { count: subs.length,     label: 'Total Renewals', color: '#475569', bg: '#F8FAFC', border: '#E2E8F0', fn: '' },
      { count: active.length,   label: 'Active',         color: '#166534', bg: '#F0FDF4', border: '#BBF7D0', fn: '' },
      { count: expiring.length, label: 'Expiring Soon',  color: '#92660F', bg: '#FAF1E0', border: '#F0C06A', fn: "scrollToSubsGroup('expiring')" },
      { count: expired.length,  label: 'Expired',        color: '#C0392B', bg: '#FDEDEC', border: '#F5C6CB', fn: "scrollToSubsGroup('expired')" },
    ].map(c => `<div
        onclick="${c.fn}"
        style="border:1.5px solid ${c.border};border-radius:10px;padding:16px;
               background:${c.bg};display:flex;flex-direction:column;
               ${c.fn ? 'cursor:pointer;' : ''}">
        <div style="font-size:30px;font-weight:800;color:${c.color};line-height:1;">${c.count}</div>
        <div style="font-size:11px;font-weight:700;color:${c.color};margin-top:5px;
          text-transform:uppercase;letter-spacing:0.06em;">${c.label}</div>
        ${c.fn ? `<div style="font-size:10px;color:${c.color};opacity:0.6;margin-top:6px;">Click to view →</div>` : ''}
      </div>`).join('');
  }
