// subscriptions.js — renewal tracking (admin + customer)
...
async function loadSubscriptions() {
  ...
  if (!subs || !subs.length) { el.innerHTML = `<div class="empty-state">No renewals yet. Click + Add Renewal to get started.</div>`; return; }
...
async function openCreateSubModal() {
  editingSubId = null;
  document.getElementById('sub-modal-title').textContent = 'Add Renewal';
...
async function openEditSubModal(subId) {
  ...
  if (!s) { alert('Could not load renewal.'); return; }

  editingSubId = subId;
  document.getElementById('sub-modal-title').textContent = 'Edit Renewal';
  document.getElementById('sub-form-error').style.display = 'none';
...
async function deleteSub(subId, name) {
  if (!confirm(`Delete renewal "${name}"?`)) return;
