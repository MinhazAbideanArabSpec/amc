...
async function loadSubscriptions() {
  ...
  if (summaryEl) {
    // Sort expiring by days left ascending, expired by days ago ascending
    const expiringList = [...expiring]
      .sort((a, b) => subStatus(a.end_date).days - subStatus(b.end_date).days)
      .slice(0, 5);
    const expiredList = [...expired]
      .sort((a, b) => subStatus(b.end_date).days - subStatus(a.end_date).days)
      .slice(0, 5);

    function summaryCard(count, label, color, bg, border, items, daysFn) {
      const listHtml = items.length ? `
        <div style="margin-top:10px;border-top:1px solid ${border};padding-top:8px;display:flex;flex-direction:column;gap:5px;">
          ${items.map(s => {
            const st = subStatus(s.end_date);
            const customer = s.profiles?.name || '—';
            const short = customer.length > 28 ? customer.substring(0,26) + '…' : customer;
            const daysText = st.days < 0 ? Math.abs(st.days) + 'd ago' : st.days + 'd left';
            return `<div style="display:flex;justify-content:space-between;align-items:center;gap:6px;">
              <span style="font-size:11px;color:${color};font-weight:600;flex:1;min-width:0;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">${short}</span>
              <span style="font-size:11px;font-weight:700;color:${color};white-space:nowrap;">${daysText}</span>
            </div>`;
          }).join('')}
        </div>` : '';

      return `
        <div style="border:1.5px solid ${border};border-radius:10px;padding:14px 16px;background:${bg};">
          <div style="font-size:28px;font-weight:800;color:${color};line-height:1;">${count}</div>
          <div style="font-size:12px;font-weight:700;color:${color};margin-top:4px;text-transform:uppercase;letter-spacing:0.05em;">${label}</div>
          ${listHtml}
        </div>`;
    }
    ...
    summaryEl.innerHTML =
      ...
      summaryCard(active.length,   'Active',         '#166534', '#F0FDF4', '#BBF7D0', [], null) +
      summaryCard(expiring.length, 'Expiring Soon',  '#92660F', '#FAF1E0', '#F0C06A', expiringList, null) +
      summaryCard(expired.length,  'Expired',        '#C0392B', '#FDEDEC', '#F5C6CB', expiredList,  null);
  }
  ...
  if (expiring.length) {
    ...
    html += `<div class="sub-grid-full" style="margin-bottom:20px;">` + expiring.map(s => subCardHtml(s, true, true, true)).join('') + `</div>`;
  }
  if (active.length) {
    ...
    html += `<div class="sub-grid-full" style="margin-bottom:20px;">` + active.map(s => subCardHtml(s, true, true, true)).join('') + `</div>`;
  }
  if (expired.length) {
    ...
    html += `<div class="sub-grid-full">` + expired.map(s => subCardHtml(s, true, true, true)).join('') + `</div>`;
  }
  el.innerHTML = html;
}
