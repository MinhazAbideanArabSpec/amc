...
function subStatus(endDate) {
  ...
  if (days <= 30) return { label: t('sub_expiring').replace('{n}', days),          cls: 'expiring', days };
  ...
  if (fullWidth) {
    return `
      <div class="sub-card sub-card-full sub-${st.cls}">
        ...
        ${s.reference_number ? `<div style="flex:1 1 100%;font-size:11.5px;color:#8A8377;margin-top:4px;">Ref: ${s.reference_number}</div>` : ''}
      ${s.notes ? `<div style="flex:1 1 100%;font-size:11.5px;color:#8A8377;margin-top:4px;">${s.notes}</div>` : ''}
      </div>`;
  }
  return `
    <div class="sub-card sub-${st.cls}">
      ...
      ${s.reference_number ? `<div style="font-size:11.5px;color:#8A8377;margin-top:4px;">Ref: ${s.reference_number}</div>` : ''}
