        ...
        ${groups[section].map(t => `
          <div class="tag-item" id="tagitem-${t.id}">
            ...
            <input type="text" id="taginput-${t.id}" value="${t.label}"
              style="display:none;flex:1;margin-bottom:0;padding:2px 6px;font-size:12px;height:24px;"
              onkeydown="if(event.key==='Enter') saveTagEdit('${t.id}'); if(event.key==='Escape') cancelTagEdit('${t.id}')"/>
            <button class="tag-item-del" title="Edit" onclick="startTagEdit('${t.id}')"
              style="color:var(--accent);margin-right:2px;">✎</button>
...
function cancelTagEdit(tagId) {
  ...
  document.getElementById(`taginput-${tagId}`).style.display = 'none';
}
...
async function saveTagEdit(tagId) {
  ...
  const { error } = await sb.from('issue_tag_definitions').update({ label: newLabel }).eq('id', tagId);
  if (error) { alert('Failed to update: ' + error.message); return; }

  // Update the label in place without full reload
