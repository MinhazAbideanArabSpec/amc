...
function renderIssueTags(assetId, section) {
  ...
  return `
    <div class="issue-checks-wrap" id="issue-wrap-${slugify(assetId + section)}">
      <div style="font-size:11px;font-weight:700;color:var(--rust);text-transform:uppercase;letter-spacing:0.05em;margin-bottom:4px;">Select Issues</div>
      ${sectionTags.map(t => `
...
function toggleIssueTags(assetId, section, result) {
  ...
  if (result === 'fail') {
    ...
    // Uncheck all tags for this section when result changes from fail
