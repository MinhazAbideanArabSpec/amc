      ...
      <div class="report-asset-block" style="margin-bottom:14px;">
        <div class="report-asset-header" style="display:flex;justify-content:space-between;align-items:center;">
          <div>
            ...
            <div style="font-size:12px;color:#8A8377;">${asset?.name || ''} · ${asset?.category || ''}</div>
          </div>
          ${overallBadge}
        </div>
            ...
            return `
              <div style="display:flex;align-items:flex-start;justify-content:space-between;
                ...
                <div style="flex:1;">
                  ...
                  ${note ? `<div style="font-size:12px;color:#8A8377;margin-top:2px;">${note}</div>` : ''}
                  ${sectionTags.length ? `<div style="margin-top:4px;">${sectionTags.map(t =>
                    `<span class="issue-tag-pill ${result === 'fail' ? 'critical' : ''}">${t.label}</span>`
                  ).join('')}</div>` : ''}
                </div>
                <span style="font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;
                  background:${resultBg};color:${resultColor};margin-left:12px;white-space:nowrap;">
                  ${result.toUpperCase()}
                </span>
              </div>`;
          }).join('')}
