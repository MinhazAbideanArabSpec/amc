...
async function openCreateReportModal() {
  ...
  reportStatusAssignments = {};

  // Load status definitions once
  const { data: statuses } = await sb.from('asset_statuses').select('*').order('name');
  allVisitStatuses = statuses || [];
...
async function openEditReportModal(reportId) {
  ...
  // Load existing status assignments for this customer's assets
  const assetIds = assets.map(a => a.id);
  const { data: existingAssignments } = await sb.from('asset_status_assignments')
    .select('asset_id, status_id, notes, is_resolved')
    .eq('customer_id', report.customer_id)
    .in('asset_id', assetIds);

  assets.forEach(a => {
    reportStatusAssignments[a.id] = {};
    (existingAssignments || []).filter(x => x.asset_id === a.id).forEach(x => {
      reportStatusAssignments[a.id][x.status_id] = {
        checked: !x.is_resolved,
        note: x.notes || ''
      };
    });
  });
  ...
  if (selectedAssets.length) {
    ...
    setTimeout(() => {
      selectedAssets.forEach(a => {
        ...
        const assignments = reportStatusAssignments[a.id] || {};
        Object.entries(assignments).forEach(([statusId, val]) => {
          const chk = document.getElementById(`vstatus-${a.id}-${statusId}`);
          if (chk) {
            chk.checked = val.checked;
            const wrap = document.getElementById(`vstatus-note-wrap-${a.id}-${statusId}`);
            if (wrap) wrap.style.display = val.checked ? 'block' : 'none';
            const noteEl = document.getElementById(`vstatus-note-${a.id}-${statusId}`);
...
function onAssetSelectionChange() {
  ...
  selectedAssets.forEach(a => {
    ...
    if (!reportStatusAssignments[a.id]) {
      reportStatusAssignments[a.id] = {};
    }
  });
...
function renderChecklistForm(assets) {
  ...
  container.innerHTML = assets.map(a => `
    <div class="report-asset-block" style="margin-top:6px;">
      ...
      ${allVisitStatuses.length ? `
        <div class="report-section-block" style="background:#FAFAF9;">
          <div class="report-section-title" style="margin-bottom:12px;">
            Asset Statuses
            <span style="font-size:10px;font-weight:400;color:#8A8377;margin-left:6px;">Check all that apply to this asset</span>
          </div>
          ${allVisitStatuses.map(st => {
            const hex = STATUS_HEX[st.color] || '#475569';
            const existing = reportStatusAssignments[a.id]?.[st.id];
            const isChecked = existing?.checked || false;
            const existingNote = existing?.note || '';
            return `
              <div style="margin-bottom:10px;">
                <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
                  <input type="checkbox" id="vstatus-${a.id}-${st.id}"
                    ${isChecked ? 'checked' : ''}
                    onchange="onVisitStatusToggle('${a.id}','${st.id}',this.checked)"
                    style="width:15px;height:15px;margin-bottom:0;flex-shrink:0;cursor:pointer;"/>
                  <span style="width:9px;height:9px;border-radius:50%;background:${hex};display:inline-block;flex-shrink:0;"></span>
                  <span style="font-size:13px;font-weight:600;color:var(--ink);">${st.name}</span>
                </label>
                <div id="vstatus-note-wrap-${a.id}-${st.id}" style="display:${isChecked ? 'block' : 'none'};margin-top:5px;padding-left:23px;">
                  <input type="text" placeholder="Note for this status (optional)"
                    id="vstatus-note-${a.id}-${st.id}"
                    value="${existingNote}"
                    onchange="onVisitStatusNoteChange('${a.id}','${st.id}',this.value)"
                    style="margin-bottom:0;font-size:12.5px;padding:6px 10px;"/>
                </div>
              </div>
            `;
          }).join('')}
...
function onVisitStatusToggle(assetId, statusId, checked) {
  if (!reportStatusAssignments[assetId]) reportStatusAssignments[assetId] = {};
  if (!reportStatusAssignments[assetId][statusId]) reportStatusAssignments[assetId][statusId] = { checked: false, note: '' };
  reportStatusAssignments[assetId][statusId].checked = checked;
  // Show/hide note field
  const noteWrap = document.getElementById(`vstatus-note-wrap-${assetId}-${statusId}`);
  if (noteWrap) noteWrap.style.display = checked ? 'block' : 'none';
}

function onVisitStatusNoteChange(assetId, statusId, value) {
  if (!reportStatusAssignments[assetId]) reportStatusAssignments[assetId] = {};
  if (!reportStatusAssignments[assetId][statusId]) reportStatusAssignments[assetId][statusId] = { checked: true, note: '' };
  reportStatusAssignments[assetId][statusId].note = value;
}
...
async function saveVisitReport() {
  ...
  for (const asset of selectedAssets) {
    ...
    // One row per section (sub_check = section name for backward compat)
  ...
  // ── Also save status assignments ──
  // Get customer_id for these assets
  const custId = customer_id;
  for (const asset of selectedAssets) {
    const assignments = reportStatusAssignments[asset.id] || {};
    for (const [statusId, val] of Object.entries(assignments)) {
      ...
      if (val.checked) {
        if (existing) {
          // Reopen if resolved, update note
          await sb.from('asset_status_assignments')
            .update({ is_resolved: false, resolved_at: null, notes: val.note || null })
            .eq('id', existing.id);
        } else {
          await sb.from('asset_status_assignments')
            .insert({ asset_id: asset.id, status_id: statusId, customer_id: custId, notes: val.note || null, is_resolved: false });
        }
      } else if (existing && !existing.is_resolved) {
        // Unchecked — mark as resolved (keeps history)
        await sb.from('asset_status_assignments')
